...
func calculateCalibration(input []string) error {
	var lineNum []string
	var lineNumAux string
	var bagOfNumbers []int
	var auxNumLine []string
	var line []string
	// sample := []string{"1abc2", "a1b2c3d4e5f", "treb7uchet"}
	var sample2 = `tmmnhlxzpj1eightldxhjnone97
	...
	for _, char := range sample2 {
		if unicode.IsDigit(char) {
			lineNum = append(lineNum, string(char))
			lineSize := len(lineNum) - 1

			if lineSize > 0 {
				auxNumLine = append(auxNumLine, lineNum[lineSize])
			} else {
				auxNumLine = append(auxNumLine, lineNum[0])
			}
		}
		if unicode.IsSpace(char) {
			if len(lineNum) > 0 {
				...
				lineNumAux = line[0] + line[len(line)-1]
				lineNumber, _ := strconv.Atoi(lineNumAux)
				bagOfNumbers = append(bagOfNumbers, lineNumber)

				println("ACTUAL NUMBER IN THE LINE", lineNumAux)
				fmt.Printf("\nBAG OF NUMBERS: %v\n", bagOfNumbers)
	...
	for _, num := range bagOfNumbers {
		sum = sum + num
	}

	fmt.Println("SUM OF LINE NUMBERS: ", sum)

	return nil
}

func calculateNumbers(input []string) error {
	var numbers []string
